...
public interface StatusRepository extends JpaRepository<Status, Long> {
    ...
    @Query(value = "select * from status  where status <> 0  order by create_at desc", nativeQuery = true)
    ...
    @Query(value = "select * from status\n" +
            ...
            "         and relationship.status = 2) or owner_id = :id", nativeQuery = true)
    Iterable<Status> findAll2(@Param("id") Long id);
}
